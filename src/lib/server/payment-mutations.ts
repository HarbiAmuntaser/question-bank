...
import { PaymentError, recheckPaymentStudent, requirePaymentStudent, requirePaymentSubject, requirePaymentCodes } from "@/lib/server/payment-scope";
...
export async function redeemSubscriptionCode(raw: unknown) {
  ...
  return paymentTransaction(async (tx) => {
    ...
    if (days !== null && (!Number.isInteger(days) || days < 1 || days > 36500)) throw new PaymentError("invalid_code_window");
    const entitlement = await tx.accessEntitlement.create({ data: {
      ...
      startsAt: now, expiresAt: days ? new Date(now.getTime() + days * 86400000) : null, isActive: true,
    } });
