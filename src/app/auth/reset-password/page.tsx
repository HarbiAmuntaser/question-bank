...
export default async function Page({ searchParams }: { searchParams: Promise<{ callbackUrl?: string; token?: string }> }) {
  ...
  return <AuthShell title="كلمة مرور جديدة"><StudentAuthForm action="reset-password" callbackUrl={safeCallbackPath(params.callbackUrl)} token={params.token} /></AuthShell>;
}
