...
export default async function Page({ searchParams }: { searchParams: Promise<{ callbackUrl?: string; token?: string }> }) {
  ...
  return <AuthShell title="استعادة كلمة المرور"><StudentAuthForm action="forgot-password" callbackUrl={safeCallbackPath(params.callbackUrl)} token={params.token} /></AuthShell>;
}
