...
export default async function Page({ searchParams }: { searchParams: Promise<{ callbackUrl?: string; token?: string }> }) {
  ...
  return <AuthShell title="إعادة إرسال رسالة التأكيد"><StudentAuthForm action="resend-verification" callbackUrl={safeCallbackPath(params.callbackUrl)} token={params.token} /></AuthShell>;
}
